...
- **`index.js`** — single entrypoint that wires all middleware and routes and owns startup sequencing (environment validation → DB connect → `runMigrations()` → seed/update admin account → load pricing config → start cron → `app.listen`). Exports the configured `app` without starting it when required by tests (`require.main === module` guard), so Jest+supertest can hit routes without starting the listener.
- **DB**: development may use SQLite (`backend/database.sqlite`, path overridable via `DB_PATH`) or PostgreSQL via `DATABASE_URL`. Production requires PostgreSQL plus the critical environment variables; tests require an explicitly safe `DATABASE_URL_TEST`. `sequelize.sync()` is not used. The sole schema authority is the ordered migration registry in `db/migrations/`, tracked by `schema_migrations`; an existing non-baselined database is verified strictly and then stopped without automatic baselining.
